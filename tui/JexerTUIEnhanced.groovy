...
class JexerTUIEnhanced {
    ...
    JexerTUIEnhanced() throws Exception {
        // Initialize TApplication
        app = new TApplication(TApplication.BackendType.XTERM)

        // Apply dark theme
        JexerTheme.applyDarkTheme(app)

        // Initialize agent registry
        agentRegistry = new AgentRegistry(AgentType.BUILD)
    }
    ...
    private void setupLspCallbacks() {
        if (!LspManager.instance.isEnabled()) {
            return
        }

        LspManager.instance.setOnClientCreated { String serverId, LSPClient client, String root ->
            // Register with sidebar LSP manager
            core.LspManager.instance.registerLsp(sessionId, serverId, client, root)
            ...
            while (running) {
                try {
                    ...
                    if (sidebarPanel && sidebarPanel.getExpanded()) {
                        ...
                        core.LspManager.instance.updateDiagnosticCounts(sessionId)
        ...
        Thread.start {
            try {
                ...
                for (file in filesToTouch) {
                    def filePath = Paths.get(currentCwd, file).toAbsolutePath().toString()
                    if (new File(filePath).exists()) {
                        try {
                            LspManager.instance.touchFile(filePath, false)
                            break
                        } catch (Exception e) {
                        }
                    }
    ...
    private void setupUI() {
        int screenWidth = app.getScreen().getWidth()
        int screenHeight = app.getScreen().getHeight()
        ...
        int activityLogHeight = screenHeight - 5  // -3 for status bar + input + 2 padding

        // Create main chat window
        mainChatWindow = app.addWindow(
        ...
        autocompletePopup = new JexerAutocompletePopup(app)
        commandInput.setAutocompletePopup(autocompletePopup)

        // Create status bar
        statusBar = new JexerStatusBar(app, mainContentWidth)
        statusBar.setX(1)
     ...
     */
    private void processUserInput(String input, List<String> mentions = []) {
    ...
    private void showHelp() {
        ...
        activityLog.appendSystemMessage('/model    - Show or change current model')
        ...
        activityLog.appendSystemMessage('Tab       - Switch agent')
        activityLog.appendSystemMessage('Ctrl+C     - Exit')
        activityLog.appendSystemMessage('Ctrl+S     - Save log')
    }
     ...
     * Cycle agent.
     */
    private void cycleAgent(int direction = 1) {
